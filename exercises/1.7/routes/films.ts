...
// GET /
router.get("/", (req, res) => {
  const films = parse(jsonDbPath, defaultFilms); // C'est un objet !!!
  if (!req.query["minDuration"] && !req.query["like"]) {
  ...
  if(req.query["like"]){
    ...
    filteredFilms = filteredFilms.filter((film) => {
      return film.title.toLowerCase().startsWith(like);
    })
  }
...
router.post("/", (req, res) => {
  ...
  const filmId = defaultFilms.reduce((maxId, film) => (film.id > maxId ? film.id : maxId), 0) + 1;
